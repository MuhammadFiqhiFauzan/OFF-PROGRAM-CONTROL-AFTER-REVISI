...
async function payItems(batchId, itemIds, hasProof = false, clientPayload = {}) {
  ...
  const proofName = `${PREFIX}-payment-${Date.now()}.pdf`;
  const proofPath = `runtime/off-program-control/payment-proofs/${batchId}/${proofName}`;
  await db.execute({
    sql: "INSERT INTO off_payment (id, batch_id, payment_no, payment_date, paid_amount, payment_method, payment_proof_path, payment_proof_name, payment_proof_mime, payment_proof_size, created_at, updated_at) VALUES (?, ?, 1, '2026-05-25', ?, ?, ?, ?, 'application/pdf', 1234, ?, ?)",
    args: [paymentId, batchId, total, methods[0], proofPath, proofName, NOW.getTime(), NOW.getTime()],
  });
  ...
  return { total, method: methods[0], isFullyPaid, ignoredClientAmount, ignoredClientMethod, proofName, proofPath, uploadedProofWasRequired: hasProof };
}
...
try {
  ...
  const first = await payItems(batchId, transferIds, false, { paidAmount: 999999, paymentMethod: "Tunai" });
  ...
  check("transfer auto proof generated", first.proofName.endsWith(".pdf") && first.proofPath.includes("/payment-proofs/"), `${first.proofName}/${first.proofPath}`);
  ...
  const second = await payItems(batchId, [tunaiId], false);
  check("tunai proof optional", second.method === "Tunai" && second.total === 3000, `${second.method}/${second.total}`);
  check("tunai auto proof generated", second.proofName.endsWith(".pdf") && second.proofPath.includes("/payment-proofs/"), `${second.proofName}/${second.proofPath}`);

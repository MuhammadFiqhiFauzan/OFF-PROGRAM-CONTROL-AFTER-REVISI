/*
 * Tujuan: Root backend status Sales History: kesiapan DB, jumlah data, dan rentang tanggal.
...
export async function GET() {
    ...
    try {
        const [database, years] = await Promise.all([
            getSalesHistoryDatabaseStatus(),
            listSalesHistoryYears(),
        ]);
        return NextResponse.json({ ok: true, database, years });
    } catch (error) {
